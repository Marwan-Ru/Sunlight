...
TempHandler::TempHandler(CityGMLHandler* gHandler):ADEHandler(gHandler)
{
	_currentTransaction=nullptr;
	_currentTransition=nullptr;
	_currentVersion=nullptr;
...
void TempHandler::startElement(std::string name, void* attributes)
{
	...
	if (name=="Version")
	{
		...
		{
			_currentVersion->_isXlink = xLinkState::UNLINKED;
	...
	{
		pushCityObject( new GenericCityObject( getGmlIdAttribute( attributes ) ) );
...
void TempHandler::endElement(std::string name)
{
	...
	if (name == "versionMember")
	{
		CityObject* tempCObj = *getCurrentCityObject();
		CityObject* child = tempCObj->getChild(0);
		...
		if(child->_isXlink==xLinkState::NONE)
		{
...
void TempHandler::endDocument()
{
	...
	{
		if(transition->from()->_isXlink == xLinkState::UNLINKED)
		{
		...
		if(transition->to()->_isXlink == xLinkState::UNLINKED)
		{
	...
	for ( temporal::Version* version : _versions)
	{
		...
		std::vector<CityObject*>* members = version->getVersionMembers();
		for (std::vector<CityObject*>::iterator it = members->begin(); it != members->end(); it++)
		{
			if ((*it)->_isXlink==xLinkState::UNLINKED)
			{
				std::string query = (*it)->getAttribute("xlink");
				std::string id = getIDfromQuery(query);
				if(!(id==""))
				{
					CityObject* target = (*getModel())->getNodeById(id);
					if (target) (*it)=target;
				} 
